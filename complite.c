...
int main(int argc, char* argv[]) {
	...
	char arrprint[100];
	printf("Please input your sorcecode :D\n");
...
void madeprint(char arrprint[]) {
	char *ptr = strtok(arrprint, " "); //띄어쓰기가 나올 때 까지 문자를 ptr에 저장하고 띄어쓰기를 NULL로 바꿈
	strcpy(&print_, &ptr); //ptr을 print_에 복사함
	if (ptr != NULL) { //ptr이 NULL이 아니라면
		ptr = strtok(NULL, ";"); //;이 나올때까지 문자를 자름
		strcpy(&printed, &ptr); //ptr을 printed에 저장해줌
		//printf("%s\n", printed);
		if (check[(int)*printed] == NULL) {
			for (int i = 0; i < 2; i++) {
				printf("\n");
			}
			printf("---------------- result ----------------\n");
